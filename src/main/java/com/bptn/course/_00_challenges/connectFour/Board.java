...
public class Board {
	...
	private char[][] board;
	private Scanner boardScanner = new Scanner(System.in);
	
	public void boardSetUp() {
        System.out.println("------ Board Set up -------");
        System.out.println("Number of rows: ");
        int rows = boardScanner.nextInt(); // receive user input
        System.out.println("Number of cols: ");
        // receive column value
        int columns = boardScanner.nextInt();
        ...
        for (char[] row : board) {
        	for (int i = 0; i<row.length;i++) {
				row[i] = '-'; // fill up each row of the board with dashes
